...
def write_csv(results, output_file):
    with open(output_file, 'w') as file:
        writer = csv.DictWriter(file, fieldnames=['table name', 'source rows', 'target rows'])
        writer.writeheader()
...
def main():
    ...
    for source_file, target_file in file_map.items():
        ...
        output_file = os.path.join(output_dir, '{}_comparison.csv'.format(source_file[:-4]))
        write_csv(results, output_file)
        print("Comparison complete for {}. Results saved to {}".format(source_file, output_file))
